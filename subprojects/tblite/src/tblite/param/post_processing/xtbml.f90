...
module tblite_param_post_processing_xtbml
   ...
   character(len=*), parameter :: k_xtbmlgeometry = "geometry", k_xtbmldensity = "density", &
      & k_xtbmlorbital = "orbital", k_xtbmlenergy = "energy", k_xtbmlconvolution = "convolution", &
      & k_xtbmla = "a", k_tensor = "tensorial-output", k_xtbml = "xtbml"

   !> Record specifying the xTB-ML features post-processing
   type, public, extends(post_processing_record) :: xtbml_record
...
subroutine load_from_toml(self, table, error)
   ...
   !> Instance of the parametrization data
   class(xtbml_record), intent(inout) :: self
   !> Data structure
...
subroutine dump_to_toml(self, table, error)
   !> Instance of the parametrization data
   class(xtbml_record), intent(in) :: self
   !> Data structure
   type(toml_table), intent(inout) :: table
   !> Error handling
   type(error_type), allocatable, intent(out) :: error
...
elemental function count_mask(mask) result(ncount)
   type(xtbml_mask), intent(in) :: mask
   integer :: ncount
   ncount = 0
end function count_mask

end module tblite_param_post_processing_xtbml

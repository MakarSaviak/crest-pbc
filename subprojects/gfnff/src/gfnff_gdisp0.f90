...
subroutine d3_gradient(dispm, nat, at, xyz, npair, pairlist, zeta_scale, radii, &
   ...
   real(wp), allocatable :: energies_serial(:), dEdcn_serial(:), gradient_serial(:, :)
...
!$ serial_inner = omp_get_max_threads() == 1
   if (serial_inner) then
      allocate(energies_serial(nat), dEdcn_serial(nat), &
         &     gradient_serial(size(gradient,1),size(gradient,2)), source=0.0_wp)
      sigma_serial = 0.0_wp
   end if

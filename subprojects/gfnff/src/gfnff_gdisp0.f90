...
!$ use omp_lib, only: omp_get_max_threads
   ...
   implicit none
   public :: d3_gradient,newD3Model,d3_workspace
   private

   !> Reusable, caller-private storage for one D3 energy/gradient evaluation.
   !> Every array is reset on every call; persistence changes allocation only.
   type :: d3_workspace
      integer :: nat = 0
      integer :: max_ref = 0
      integer :: ncomp = 0
      ...
      real(wp), allocatable :: energies_serial(:), dEdcn_serial(:)
      real(wp), allocatable :: gradient_serial(:, :)
   contains
      procedure :: prepare => d3_workspace_prepare
      procedure :: release => d3_workspace_release
   end type d3_workspace
...
subroutine d3_workspace_prepare(self, nat, max_ref, ncomp, serial_inner)
   class(d3_workspace), intent(inout) :: self
   integer, intent(in) :: nat, max_ref, ncomp
   logical, intent(in) :: serial_inner
   logical :: layout_ok, serial_layout_ok

   layout_ok = allocated(self%gw) .and. allocated(self%dgwdcn) .and. &
      & allocated(self%c6) .and. allocated(self%dc6dcn) .and. &
      & allocated(self%energies) .and. allocated(self%dEdcn)
   if (layout_ok) then
      layout_ok = self%nat == nat .and. self%max_ref == max_ref .and. &
         & self%ncomp == ncomp .and. size(self%gw, 1) == max_ref .and. &
         & size(self%gw, 2) == nat .and. &
         & size(self%dgwdcn, 1) == max_ref .and. &
         & size(self%dgwdcn, 2) == nat .and. &
         & size(self%c6, 1) == nat .and. size(self%c6, 2) == nat .and. &
         & size(self%dc6dcn, 1) == nat .and. &
         & size(self%dc6dcn, 2) == nat .and. &
         & size(self%energies) == nat .and. size(self%dEdcn) == nat
   end if

   if (.not.layout_ok) then
      call self%release()
      allocate(self%gw(max_ref, nat), self%dgwdcn(max_ref, nat), &
         & self%c6(nat, nat), self%dc6dcn(nat, nat), &
         & self%energies(nat), self%dEdcn(nat))
      self%nat = nat
   ...
   if (serial_inner) then
      serial_layout_ok = allocated(self%energies_serial) .and. &
         & allocated(self%dEdcn_serial) .and. allocated(self%gradient_serial)
      if (serial_layout_ok) then
         serial_layout_ok = size(self%energies_serial) == nat .and. &
            & size(self%dEdcn_serial) == nat .and. &
            & size(self%gradient_serial, 1) == ncomp .and. &
            & size(self%gradient_serial, 2) == nat
      end if
      if (.not.serial_layout_ok) then
         if (allocated(self%energies_serial)) deallocate(self%energies_serial)
         if (allocated(self%dEdcn_serial)) deallocate(self%dEdcn_serial)
         if (allocated(self%gradient_serial)) deallocate(self%gradient_serial)
         allocate(self%energies_serial(nat), self%dEdcn_serial(nat), &
            & self%gradient_serial(ncomp, nat))
      end if
   end if

   ! Preserve the old allocate(..., source=0.0_wp) initialization on every
   ! evaluation. The serial-only buffers retain the old T1-only behavior.
   self%gw = 0.0_wp
   self%dgwdcn = 0.0_wp
   self%c6 = 0.0_wp
   self%dc6dcn = 0.0_wp
   self%energies = 0.0_wp
   self%dEdcn = 0.0_wp
   if (serial_inner) then
      self%energies_serial = 0.0_wp
      self%dEdcn_serial = 0.0_wp
      self%gradient_serial = 0.0_wp
   end if
end subroutine d3_workspace_prepare

subroutine d3_workspace_release(self)
   class(d3_workspace), intent(inout) :: self

   if (allocated(self%gw)) deallocate(self%gw)
   if (allocated(self%dgwdcn)) deallocate(self%dgwdcn)
   if (allocated(self%c6)) deallocate(self%c6)
   if (allocated(self%dc6dcn)) deallocate(self%dc6dcn)
   if (allocated(self%energies)) deallocate(self%energies)
   if (allocated(self%dEdcn)) deallocate(self%dEdcn)
   if (allocated(self%energies_serial)) deallocate(self%energies_serial)
   if (allocated(self%dEdcn_serial)) deallocate(self%dEdcn_serial)
   if (allocated(self%gradient_serial)) deallocate(self%gradient_serial)
   self%nat = 0
   self%max_ref = 0
   self%ncomp = 0
end subroutine d3_workspace_release
...
!$ serial_inner = omp_get_max_threads() == 1
   if (serial_inner) then
      ...
      !$omp shared(dispm, nat, atoms, wf, cn, gwvec, gwdcn) &
      !$omp private(iat, ati, iref, norm, dnorm, gw, expw, expd, gwk, dgwk)
      do iat = 1, nat
         ati = atoms(iat)
         norm = 0.0_wp
         dnorm = 0.0_wp
...
!$ serial_inner = omp_get_max_threads() == 1
   if (serial_inner) then
      ...
      !$omp parallel do default(none) schedule(static) &
      !$omp shared(dispm, nat, atoms, gwvec, gwdcn, c6, dc6dcn) &
      !$omp private(iat, jat, ati, atj, iref, jref, refc6, dc6, dc6dcni, dc6dcnj)
      do iat = 1, nat
         ati = atoms(iat)
         do jat = 1, iat
            atj = atoms(jat)
...
subroutine d3_gradient(dispm, nat, at, xyz, npair, pairlist, zeta_scale, radii, &
      & r4r2, weighting_factor, dispscale, cn, dcndr, energy, gradient, work, frozen_mask)
   use disp_dftd3param
   type(TDispersionData), intent(in) :: dispm
   type(d3_workspace), intent(inout) :: work
   integer, intent(in) :: nat
   ...
   real(wp), allocatable :: c6(:, :), dc6dcn(:, :)
   real(wp), allocatable :: energies(:), dEdcn(:)
   real(wp) :: sigma_serial(3, 3)
   logical :: serial_inner

   serial_inner = .true.
!$ serial_inner = omp_get_max_threads() == 1
   max_ref = maxval(dispm%nref(at))
   call work%prepare(nat, max_ref, size(gradient, 1), serial_inner)
   ! OpenMP reductions do not accept derived-type components or pointer
   ! aliases with GNU Fortran. Keep all six hot arrays as the same contiguous
   ! local allocatables used by the original implementation, but transfer
   ! ownership from and back to caller-private persistent storage at zero copy.
   call move_alloc(work%gw, gw)
   call move_alloc(work%dgwdcn, dgwdcn)
   call move_alloc(work%c6, c6)
   call move_alloc(work%dc6dcn, dc6dcn)
   call move_alloc(work%energies, energies)
   call move_alloc(work%dEdcn, dEdcn)
   if (serial_inner) then
      sigma_serial = 0.0_wp
   end if
   ...
   if (present(frozen_mask)) then
      if (serial_inner) then
         associate(energies_serial => work%energies_serial, &
            & dEdcn_serial => work%dEdcn_serial, &
            & gradient_serial => work%gradient_serial)
         do img = 1, npair
            iat = pairlist(1, img)
            jat = pairlist(2, img)
            ij = jat + iat*(iat-1)/2
            ...
            energies_serial(iat) = energies_serial(iat) + dE
            dEdcn_serial(iat) = dEdcn_serial(iat) - dc6dcn(iat, jat) * disp
            if (iat /= jat) then
               energies_serial(jat) = energies_serial(jat) + dE
               dEdcn_serial(jat) = dEdcn_serial(jat) - dc6dcn(jat, iat) * disp
               ...
               if (.not.(frozen_mask(iat).and.frozen_mask(jat))) then
                  ...
                  if (.not.frozen_mask(iat)) gradient_serial(:, iat) = gradient_serial(:, iat) + dG
                  if (.not.frozen_mask(jat)) gradient_serial(:, jat) = gradient_serial(:, jat) - dG
               end if
            end if
         enddo
         energies = energies+energies_serial
         dEdcn = dEdcn+dEdcn_serial
         gradient = gradient+gradient_serial
         end associate
      else
      ...
      if (serial_inner) then
         ...
         do img = 1, npair
            ...
            energies_serial(iat) = energies_serial(iat) + dE
            dEdcn_serial(iat) = dEdcn_serial(iat) - dc6dcn(iat, jat) * disp
            sigma_serial = sigma_serial + dS
            if (iat /= jat) then
               energies_serial(jat) = energies_serial(jat) + dE
               dEdcn_serial(jat) = dEdcn_serial(jat) - dc6dcn(jat, iat) * disp
               gradient_serial(:, iat) = gradient_serial(:, iat) + dG
               gradient_serial(:, jat) = gradient_serial(:, jat) - dG
               sigma_serial = sigma_serial + dS
            endif
         ...
         dEdcn = dEdcn+dEdcn_serial
         gradient = gradient+gradient_serial
         ! `sigma` is not consumed (the corresponding dgemv is disabled).
         ! Do not read/combine its historically uninitialized shared value.
         end associate
      else
   ...
   call move_alloc(gw, work%gw)
   call move_alloc(dgwdcn, work%dgwdcn)
   call move_alloc(c6, work%c6)
   call move_alloc(dc6dcn, work%dc6dcn)
   call move_alloc(energies, work%energies)
   call move_alloc(dEdcn, work%dEdcn)

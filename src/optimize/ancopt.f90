...
!!***********************************************************************
    ...
    real(wp) :: rot(3),gnorm
    integer :: n3,i,j,k,l,jjj,ic,jc,ia,ja,ii,jj,info,nat3,nfree3
...
!>--- initialize OPT object
    ...
    if (calc%nfreeze > 0) then
      ...
      allocate (h(nat3,nat3),hess(nat3*(nat3+1)/2))
    end if
...
!>--- ANC generation
      if (calc%nfreeze > 0) then
        ...
        do i=1,nat3
          do j=1,i
            k=k+1; h(i,j)=hess(k); h(j,i)=hess(k)
          end do
        end do
...
!========================================================================================!
!> Diagnostic dump used only by the AUGUR projected-ANCOPT benchmark.
subroutine dump_anc_debug(OPT)
  type(optimizer), intent(in) :: OPT
  integer :: u, i

  open(newunit=u,file='anc_eigvals.bin',access='stream',form='unformatted',status='replace')
  write(u) OPT%n3, OPT%nvar
  do i = 1, OPT%nvar
    write(u) OPT%hess(i+i*(i-1)/2)
  end do
  close(u)

  open(newunit=u,file='anc_modes.bin',access='stream',form='unformatted',status='replace')
  write(u) OPT%n3, OPT%nvar
  write(u) OPT%B(:,1:OPT%nvar)
  close(u)
end subroutine dump_anc_debug

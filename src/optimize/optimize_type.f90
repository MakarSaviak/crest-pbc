...
module optimize_type
  ...
  type optimizer
    !******************************************************
    !* Storage type for optimizations
    !*
    !* Depending on the chosen optimization algorithm
    !* different variables in this will be used/allocated
    !******************************************************
  ...
  contains
    ...
    procedure :: allocatelbfgs => allocate_lbfgs
...
!========================================================================================!
  ...
  subroutine allocate_opt(self,n)
    implicit none
    class(optimizer),intent(inout)  :: self
    integer,intent(in) :: n
    integer            :: nvar
  ...
  subroutine allocate_lbfgs(self,nvar,mem)
    implicit none
    class(optimizer),intent(inout)  :: self
    integer,intent(in) :: nvar,mem
    call self%deallocate
    self%m = mem
    self%nvar = nvar
    allocate(self%S(nvar,mem))
    allocate(self%Y(nvar,mem))
    allocate(self%rho(mem))
    allocate(self%alpha(mem))
    allocate(self%q(nvar))

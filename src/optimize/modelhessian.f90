...
!========================================================================================!
!> Construct only the mobile/mobile block of the Lindh model Hessian.
!> Host-mobile interactions still contribute to mobile diagonal curvature, while
!> entries containing any frozen coordinate are discarded during accumulation.
  subroutine modhes_free(calc,modh,natoms,xyz,at,freezelist,Hess,pr)
    ...
    integer :: nfree3,np,i,j,p,ii,jj,q,atom,c
    integer,allocatable :: idx(:)
    real(wp),allocatable :: work(:),fullcons(:)
    ...
    select case (modh%model)
    case default
      if (pr) write (*,'(a)') 'Using direct free/free Lindh-Hessian (1995)'
      call ddvopt_free(xyz,natoms,work,at,modh,freezelist)
    end select
    Hess = work(1:np)
    deallocate(work)

    ! Exact fallback for explicit non-freeze constraints. Pure $fix-atoms jobs
    ! have calc%nconstraints == 0 and avoid any full packed allocation.
    if (calc%nconstraints > 0) then
      allocate(fullcons((3*natoms)*(3*natoms+1)/2),source=0.0_wp)
      call constrhess(natoms,at,xyz,calc,fullcons)
      ...
      do i=1,nfree3
        ...
        do j=1,i
          ...
          Hess(p)=Hess(p)+fullcons(q)
        end do
      end do
      deallocate(idx,fullcons)
    end if

...
subroutine create_neigh(nat,xyz_rad,neigh_list,neigh_index,neigh_type)
  ...
  do i = 1,nat
    ...
    do j = 1,nat
      if (j .NE. i) then
        if (abs(x-xyz_rad(j,1)) .lt. r+xyz_rad(j,4)) then
          d = sqrt((x-xyz_rad(j,1))**2+(y-xyz_rad(j,2))**2+(z-xyz_rad(j,3))**2)
          ri = xyz_rad(j,4)
          if (d .lt. r+ri) then
            if (d+r .LE. ri) then
              neigh_list(i) = -1
...
subroutine generate_integration_parts(single_sphere,circles,nat,num_neigh,int_parts,num_parts)
  ...
  do i = 1, (num_neigh)
    ...
    circles(i,1) = -a/(2d0*c)
    circles(i,2) = -b/(2d0*c)
    circles(i,3) = sqrt((a**2+b**2-4d0*c*d)/(4d0*c**2))
    if (c .gt. 0) then
      circles(i,4) = -1
    else
      circles(i,4) = 1
...
subroutine make_parts(num,circles,nat,num_neigh,no_arc,int_partsnew)
  ...
  do i = 1, (num_neigh)
    if (i .NE. num) then
      ...
      if ((dist .lt. c23+c13).and.(abs(c23-c13) .lt. dist)) then
        !--- Two intersections
  ...
  if (counter_angles .eq. 0) then
    ...
    do i = 1, (num_neigh)
      if (i .NE. num) then
        ...
        dist = sqrt((circles(num,1)+circles(num,3)-circles(i,1))**2+ &
        ...
        elseif (dist .gt. circles(i,3)) then
          ...
          dist = sqrt((circles(num,1)-circles(i,1))**2+(circles(num,2)-circles(i,2))**2)
          if (dist .lt. circles(i,3)) then
            if (circles(i,4) .gt. 0) then
              counter = 1
            else
    ...
    do i = 2,counter_angles
      if (abs(angles(i)-angles(i-1)) .gt. 1d-12) then
        m = m+1
    ...
    do i = 1, (counter_angles-1)
      ...
      do j = 1, (num_neigh)
        if (j .NE. num) then
          c21 = c11+c13*cos((angles(i)+angles(i+1))/2d0)
          c22 = c12+c13*sin((angles(i)+angles(i+1))/2d0)
          !--- Check, if point is inside circle
          dist = sqrt((c21-circles(j,1))**2+(c22-circles(j,2))**2)
    ...
    do i = 1, (num_neigh)
      if (i .NE. num) then
        c21 = c11+c13*cos((angles(1)+2d0*pi+angles(counter_angles))/2d0)
        c22 = c12+c13*sin((angles(1)+2d0*pi+angles(counter_angles))/2d0)
        !--- Check, if point is inside circle
        dist = sqrt((c21-circles(i,1))**2+(c22-circles(i,2))**2)
        if (dist .lt. circles(i,3)) then
          if (circles(i,4) .gt. 0) then
            counter_no_angles = counter_no_angles+1
          end if
...
subroutine intersection(point1,point2,nat,circles,int11,int12,int21,int22)
  ...
  if (abs(c21-c11) .lt. 1d-12) then
    ...
    if (f1 .eq. 0) then
      ...
      int22 = pi
    elseif (f1 .gt. 0) then
      int21 = atan(abs(f1/f2))
      int22 = pi-int21
    else
      int21 = pi+atan(abs(f1/f2))
      int22 = 3d0*pi-int21
    end if
    f1 = f1+c22-c12
    if (f1 .eq. 0) then
      ...
      int12 = pi
    elseif (f1 .gt. 0) then
      int11 = atan(abs(f1/f2))
      int12 = pi-int11
    else
      int11 = pi+atan(abs(f1/f2))
      int12 = 3d0*pi-int11
    end if
  else
    f3 = ((c13**2-c23**2-(c22-c12)**2)/(c21-c11)-(c21-c11))/2d0
    f4 = (c12-c22)/(c21-c11)
    f1 = (-f3*f4+sqrt((f4**2+1d0)*c23**2-f3**2))/(f4**2+1d0)
    f2 = f3+f4*f1
    if (f2 .eq. 0) then
      if (f1 .gt. 0) then
        int21 = pi/2d0
...
subroutine integrate(circles,int_parts,nat,nint_parts,rad,z1,av_part)
  ...
  do i = 1,nint_parts
    ...
    pre_A = sqrt(pre_V**2-xz**2-yz**2)
    f = z**2-pre_V
    if (abs(abs(int_parts(i,3))-2d0*pi) .lt. 1d-12) then
      v1 = 2d0*pi/pre_A
      ...
      v1 = 2d0*(pi/2d0-atan((pre_V*cos((p1-p2)/2d0)+ &
      &   xz*cos((p2+p1)/2d0)+yz*sin((p2+p1)/2d0))/ &
      &   (pre_A*sin((p1-p2)/2d0))))/pre_A
      pa = sin(p1)
      pb = cos(p1)
      pc = sin(p2)
      pd = cos(p2)

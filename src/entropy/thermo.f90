...
!========================================================================================!
  ...
  subroutine thermodyn(iunit,A_rcm,B_rcm,C_rcm,avmom_si,linear,atom,sym,molmass, &
        &              vibs,nvibs,T,sthr_rcm,et,ht,g,ts,zp,pr,emodel)
    use iso_fortran_env,only:wp => real64
    implicit none
    ...
    integer,intent(in),optional :: emodel
    ...
    if (present(emodel)) then
      !> selecting vib. entropy approximation: 1=Grimme (2012), 2=Truhlar (2011)
      model = emodel
    else
      model = 1
    end if
    ...
    do i = 1,nvibs
      select case (model)
      case (1) !> Grimme mod.RRHO (2012)
        omega = vibs(i)
      case (2) !> Truhlar cutoff (2011)
        if (vibs(i) >= 0.0_wp) then
          omega = max(sthr,vibs(i))
        else
          omega = vibs(i)
        end if
      end select
      ...
      if (omega .gt. 0) then
        ...
        ! harm. osc. entropy
        sv_ho = omega*beta*ewj/(1.0_wp-ewj)-log(1.0_wp-ewj)
        ! free rotor entropy
  ...
  subroutine print_thermo_sthr_cut(iunit,nvib,vibs,sthr_rcm,temp)
    implicit none
    ...
    integer  :: i
    real(wp) :: maxfreq,omega,s_r,s_v,omega_cut
    real(wp) :: beta,ewj,mu,RT,sthr,avmom,ewj_cut
    beta = 1.0_wp/kB/temp ! beta in 1/Eh
    sthr = sthr_rcm*rcmtoau ! sthr in Eh
    RT = kb*temp*autokcal ! RT in kcal/mol for printout
    ...
    write (iunit,'(a)')
    maxfreq = sthr_rcm
    write (iunit,'(1x,a,f6.2,a,/)') 'Frequencies up to ',maxfreq,' cm⁻¹ will '// &
    & 'crudely be cut to exactly this value (Truhlar, 2011)'
    write (iunit,'(a8,a14,1x,a27,a27,a13)') &
      "mode","ω/cm⁻¹","T·S(HO)/kcal·mol⁻¹","T·S(cut)/kcal·mol⁻¹","ΔT·S"
    write (iunit,'(3x,72("-"))')
    do i = 1,nvib
      ! frequency is Eh
      omega = vibs(i)
      ...
      ! omega in Eh, beta in 1/Eh
      ewj = exp(-omega*beta)
      ewj_cut = exp(-omega_cut*beta)

      if (omega .gt. 0) then
        ! this is S/R which is dimensionless
        s_v = omega*beta*ewj/(1.0_wp-ewj)-log(1.0_wp-ewj)
        s_r = omega_cut*beta*ewj_cut/(1.0_wp-ewj_cut)-log(1.0_wp-ewj_cut)
      else
        s_v = 0.0_wp
        s_r = 0.0_wp
      end if
      write (iunit,'(i8,f10.2,9x,f12.5,10x,f12.5,1x,f12.5)') &
        i,omega*autorcm,-RT*s_v,-RT*s_r,-RT*(s_v-s_r)
    end do
    write (iunit,'(3x,72("-"))')

  end subroutine print_thermo_sthr_cut

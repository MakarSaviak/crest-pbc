...
contains
  ...
  subroutine load_nci_input_ensemble(env,prepared_ref,mols,ninputs)
    ...
    real(wp) :: rot(3),avmom,evec(3,3),center(3)
    ...
    ! Reproduce the transform applied by the normal positional input reader to
    ! frame 1 and reuse it for all placements.  Independent alignment would move
    ! the common frozen host and destroy the supplied placement relationship.
    ...
    ! axis() may flip the first eigenvector to preserve chirality. Determine and
    ! reuse that exact convention instead of aligning each placement separately.
    allocate (xyz_bohr(3,nat,ninputs))
    do iatom = 1,nat
      shifted = xyz_ang(:,iatom,1)-center
      xyz_bohr(:,iatom,1) = matmul(transpose(evec),shifted)
    end do
    if (maxval(abs(xyz_bohr(:,:,1)-aligned_first)) > transform_tolerance) then
      evec(:,1) = -evec(:,1)
    end if
    do iframe = 1,ninputs
      do iatom = 1,nat
    ...
    if (max_transform_delta > transform_tolerance) then
      error stop 'Multi-input NCI: failed to reconstruct the common axis transform.'
    end if
    ...
    if (nfixed > 0) then
      write (stdout,'(1x,a,es12.4,a)') 'Maximum frozen-atom deviation : ', &
        max_fixed_delta,' Angstrom'
    end if
  ...
  subroutine preoptimize_additional_inputs(env,mols,input_topology)
    ...
    write (stdout,'(/,1x,a,i0,a)') 'Preoptimizing ',size(mols)-1, &
      ' additional NCI input structures with one calculator core each.'
